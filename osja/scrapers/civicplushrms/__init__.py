...
class CivicPlusHrmsJobScraper(JobScraper):
    def __init__(self):
        super(CivicPlusHrmsJobScraper, self).__init__()
        self.headers = {
    ...
    def scrape_job_links(self):
        ...
        ajax_url = urljoin(self.url, 'CareerPortal.ashx')
        ...
        def build_job_url(source_id, requisition_id):
            ...
            req = Request('GET', ajax_url, params=params).prepare()
            return req.url

        self.params['_'] = int(time.time())
        self.params['clientId'] = client_id

        resp = self.session.get(ajax_url, headers=self.headers, params=self.params)
        data = resp.json()
